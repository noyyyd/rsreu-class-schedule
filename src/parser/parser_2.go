...
func ParseSchedule2(filePath string) ([]*entities.Schedule, error) {
	...
	for {
		...
		if err != nil {
			log.Printf("failed get lesson time %s: %v", filePath, err)
			break
		}
		weekType, err := getWeekType(file, sheet, axis)
		if err != nil {
			log.Printf("failed get week type %s: %v", filePath, err)
			break
		}
		...
		for _, schedule := range schedules {
			...
			if err != nil {
				...
				log.Printf("failed get lesson info %s: %v", filePath, err)
				break
			}
